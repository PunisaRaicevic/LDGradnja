...
import { useConstructionLogFileStore } from '@/store/useConstructionLogFileStore';
...
import {
  ...
  FileSpreadsheet, FileText, Image, FileIcon, BookOpen,
} from 'lucide-react';
...
export default function ConstructionDiary() {
  ...
  const { files, loading, loadFiles, addFile, deleteFile, getFile, getSignedUrl } = useConstructionLogFileStore();
  ...
  const [previewFile, setPreviewFile] = useState<ConstructionLogFile | null>(null);
  ...
  const handleFileChanged = useCallback(async (data: { fileName: string }) => {
    ...
    try {
      // Read the file content from temp directory via IPC
      const api = (window as any).electronAPI;
      const result = await api.readTempFile(data.fileName);
      if (!result.success) {
        setSyncMessage(`Greška: ${result.error}`);
        return;
      }

      // result.buffer is a Node.js Buffer (serialized as Uint8Array via IPC)
      const uint8 = new Uint8Array(result.buffer);
      const blob = new Blob([uint8], { type: 'application/octet-stream' });

      setSyncMessage(`Sinhronizujem (${(blob.size / 1024).toFixed(0)} KB)...`);

      const { error } = await supabase.storage.from('construction-log-files').update(file.filePath, blob);
      if (error) {
        setSyncMessage(`Greška: ${error.message}`);
      } else {
        await supabase.from('construction_log_files').update({ file_size: blob.size }).eq('id', file.id);
  ...
  const handlePreview = async (file: ConstructionLogFile) => {
    setPreviewFile(file);
  ...
  const handleDownload = async (file: ConstructionLogFile) => {
    const blob = await getFile(file.id);
    if (blob) {
  ...
  return (
    <div>
      <div className="flex flex-wrap items-center justify-between gap-2 mb-6">
        <h1 className="text-xl lg:text-2xl font-bold">Građevinska knjiga</h1>
        ...
        <Card>
          <CardContent className="py-12 text-center">
            ...
            <p className="text-sm text-muted-foreground mt-1">
              Uploadujte PDF, Excel, Word ili slike građevinske knjige
            </p>
          </CardContent>
        </Card>
      ) : (
      ...
      <Dialog open={uploadOpen} onOpenChange={(open) => { if (!open) { setUploadOpen(false); setPendingFiles([]); setDescription(''); } }}>
        <DialogContent onClose={() => { setUploadOpen(false); setPendingFiles([]); setDescription(''); }}>
          <DialogHeader>
            <DialogTitle>Upload građevinske knjige</DialogTitle>
          </DialogHeader>
