...
import { useSituationFileStore } from '@/store/useSituationFileStore';
...
import {
  ...
  FileSpreadsheet, FileText, Image, FileIcon, ClipboardList,
} from 'lucide-react';
...
export default function InterimSituations() {
  ...
  const { files, loading, loadFiles, addFile, deleteFile, getFile, getSignedUrl } = useSituationFileStore();
  ...
  const [previewFile, setPreviewFile] = useState<SituationFile | null>(null);
  ...
  const handlePreview = async (file: SituationFile) => {
    setPreviewFile(file);
  ...
  const handleDownload = async (file: SituationFile) => {
    const blob = await getFile(file.id);
    if (blob) {
    ...
    <div>
        ...
        <Card>
          <CardContent className="py-12 text-center">
            ...
            <p className="text-sm text-muted-foreground mt-1">
              Uploadujte PDF, Excel, Word ili slike privremenih situacija
            </p>
          </CardContent>
        </Card>
      ) : (
      ...
      <Dialog open={uploadOpen} onOpenChange={(open) => { if (!open) { setUploadOpen(false); setPendingFiles([]); setDescription(''); } }}>
        <DialogContent onClose={() => { setUploadOpen(false); setPendingFiles([]); setDescription(''); }}>
          <DialogHeader>
            <DialogTitle>Upload privremene situacije</DialogTitle>
          </DialogHeader>
